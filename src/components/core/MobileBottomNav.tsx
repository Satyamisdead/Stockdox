...
import { Home, Search, User, Eye, LogOut } from "lucide-react"; 
...
export default function MobileBottomNav() {
  ...
  const navItemsBase = [
    ...
    { id: 'search', href: "/search-mobile", label: "Search", icon: Search, requiresAuth: false },
  ];
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-background border-t border-border/60 shadow-md md:hidden z-50">
      <div className="flex justify-around items-center h-full">
        {navItems.map((item) => {
          const isActive = item.href ? (pathname === item.href || (item.href === "/watchlist" && pathname.startsWith("/asset"))) : false;
