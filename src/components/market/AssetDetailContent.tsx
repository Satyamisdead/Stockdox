...
export default function AssetDetailContent({ initialAsset }: AssetDetailContentProps) {
  ...
  // Effect to fetch live data on the client side
  useEffect(() => {
    const fetchLiveData = async () => {
      ...
      try {
        ...
        // Update state with the new live data, falling back to initial data
        setAsset(prevAsset => ({
          ...prevAsset,
        ...
        // Silently fail, leaving placeholder/initial data in place.
      } finally {
        setIsFetchingLiveData(false);
      }
    };
    
    // Run the fetch operation on component mount
    fetchLiveData();
