...
export default function BitcoinMiniChartWidget({ size = 'md', currentPrice, currentChangePercent }: BitcoinMiniChartWidgetProps) {
  ...
  return (
    <Link href="/asset/bitcoin" className="block">
      <Card className={cn(
        "border-primary/50 bg-black/40 backdrop-blur-sm shadow-inner flex flex-col cursor-pointer hover:border-primary transition-colors border-2",
        ...
        <CardContent className={cn("flex flex-col flex-1 pt-0", isSmall ? "p-1.5" : "p-2")}>
          <div className="flex items-center justify-between mb-0.5 shrink-0">
            ...
            <p className={cn(
              isPositive ? 'text-yellow-400' : 'text-red-400',
               isSmall ? "text-[9px]" : "text-xs"
            )}>{displayChange}</p>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={bitcoinChartData} margin={{ top: 5, right: 0, left: 0, bottom: 5 }}>
                ...
                <Line
                  ...
                  dot={{ r: isSmall ? 1 : 1.5, fill: 'hsl(var(--primary))', strokeWidth: 0 }}
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
